...
def test_designer_progression_resolves_teams_in_scorecard(live_server, page: Page):
    ...
    slot_sf1 = TemplateSlot.objects.create(
        ...
        stage="Final",
        standing="SF1",
        home_reference="Winner Gruppe A",
        away_reference="Runner-up Gruppe B",
        official_group=0,
        official_team=1,
    )

    # Update Rule for SF1 (depends on Vorrunde stage completion)
    rule_sf1 = TemplateUpdateRule.objects.create(
        template=template, slot=slot_sf1, pre_finished="Vorrunde"
    )
    TemplateUpdateRuleTeam.objects.create(
        update_rule=rule_sf1, role="home", standing="Gruppe A", place=1
    )
    TemplateUpdateRuleTeam.objects.create(
        update_rule=rule_sf1, role="away", standing="Gruppe B", place=2
    )
    ...
    # Apply Template
    TemplateApplicationService(
        template, gameday, team_mapping, applied_by=admin
    ).apply()
    ...
    _finalize_game_via_api(
        live_server.url, token, gi_b, home_score=0, away_score=12
    )  # B2 wins, B1 is 2nd
    ...
    expect(
        page.get_by_text("Winner Gruppe A vs Runner-up Gruppe B")
    ).not_to_be_visible()
