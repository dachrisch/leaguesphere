...
describe('MetadataTeamPoolRow top-row card consistency', () => {
  ...
  it('team pool card uses the metadata accordion chrome (no Card frame)', () => {
    ...
    // … and the same status-tinted header background (DRAFT → warning).
    expect(poolCard.querySelector('.accordion-header')).toHaveClass('header-status-warning');
  ...
  it('swiss control card shares the identical accordion chrome + status header', async () => {
    const { container } = render(
      <MetadataTeamPoolRow
        {...createRowProps()}
        swissPanel={<SwissStandingsPanel gamedayId={1} status="DRAFT" />}
      />,
    );
    ...
    expect(swissCard.querySelector('.accordion-header')).toHaveClass('header-status-warning');

    // Identical Card classNames across all three top-row cards.
    ...
    const headerClasses = (el: Element) =>
      Array.from(el.classList)
        ...
        .sort();
    const reference = headerClasses(screen.getByTestId('gameday-metadata-header'));
    expect(headerClasses(screen.getByTestId('team-pool-card').querySelector('.accordion-header')!)).toEqual(
      reference,
    );
    expect(headerClasses(swissCard.querySelector('.accordion-header')!)).toEqual(reference);
  });
