...
show_help() {
    ...
    echo "  When deploying from master (or via -b flag), a release branch is created"
    echo "  and automatically merged back via PR (if gh CLI is available)."
    echo "  Existing PRs are detected and skipped."
    echo
    echo "Examples:"
    echo "  $0 stage                      # Current branch (staging/RC)"
    echo "  $0 demo                       # Current branch (demo)"
    echo "  $0 -b upstream/master patch   # Worktree from upstream/master (creates PR)"
    echo "  $0 -b origin/master stage     # RC version via worktree (creates PR)"
...
create_release_pr() {
    ...
    if ! command -v gh &> /dev/null; then
        ...
        echo "Manual PR creation needed: $release_branch → $source_branch"
        return 1
    fi
...
This PR was auto-generated by the deploy script."
    ...
    if gh pr create \
        ...
        --head "$release_branch" \
        --base "$source_branch" \
        --repo "$(git config --get remote.${remote}.url)" 2>/dev/null; then
        echo "✅ PR created successfully for $release_branch"
        return 0
    else
...
case "$VERSION_ARG" in
    major|minor|patch)
        ...
        if git push -u $REMOTE HEAD:refs/heads/$RELEASE_BRANCH && git push $REMOTE --tags; then
            ...
            create_release_pr "$RELEASE_BRANCH" "$DEPLOY_BRANCH" "$REMOTE" "$NEW_VERSION"
        fi
    ...
    stage)
        ...
        # Push commits and tags to release branch
        if git push -u $REMOTE HEAD:refs/heads/$RELEASE_BRANCH && git push $REMOTE --tags; then
            # Create PR for release branch if it was created
