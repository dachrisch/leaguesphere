...
This PR was auto-generated by the deploy script."
    ...
    # Extract owner from push remote URL for cross-fork PRs
    local push_owner=$(git config --get remote.${push_remote}.url | grep -oP '(?:github\.com[:/])\K[^/]+' || echo "")
    local head_ref="$release_branch"
    if [ -n "$push_owner" ] && [ "$push_remote" != "$pr_remote" ]; then
        head_ref="$push_owner:$release_branch"
    fi
...
# Create a draft GitHub release for a pushed tag.
#
# release-please creates a draft release for every tag it produces; CI then flips
# that draft to prerelease (promote_to_prerelease) or to latest
# (create_production_release) via `gh release edit`. When we tag from this script
# instead of release-please, no draft exists, so that `gh release edit` fails with
# "release not found". This mirrors release-please so the CI promote step succeeds.
create_draft_release() {
    local tag="$1"          # e.g. v3.28.5-rc.1 (already pushed to push_remote)
    local push_remote="$2"

    if ! command -v gh &> /dev/null; then
        echo "⚠️  gh CLI not found - skipping draft release creation for $tag"
        echo "   CI 'gh release edit' will fail until a draft release exists for $tag"
        return 1
    fi

    # Derive owner/repo from the remote the tag was pushed to
    local repo
    repo=$(git config --get remote.${push_remote}.url | sed -E 's#.*github\.com[:/]##; s#\.git$##')
    local repo_arg=()
    ...
    echo "Creating draft release $tag (mirrors release-please)..."
    if gh release create "$tag" "${repo_arg[@]}" --draft --title "$tag" --generate-notes 2>/dev/null \
        || gh release create "$tag" "${repo_arg[@]}" --draft --title "$tag" \
             --notes "Automated draft release for $tag (created by deploy.sh)." 2>/dev/null; then
        echo "✅ Draft release created: $tag"
        return 0
    else
        echo "⚠️  Failed to create draft release $tag (CI promote/create-release will fail)"
        return 1
    fi
}
...
case "$VERSION_ARG" in
    major|minor|patch)
        ...
        # promote_to_prerelease / create_production_release (gh release edit) have
        # a release to flip to prerelease / latest.
        create_draft_release "v$NEW_VERSION" "$REMOTE"
    ...
    stage)
        ...
        # promote_to_prerelease (gh release edit) has a release to flip.
        create_draft_release "v$NEW_VERSION" "$REMOTE"
