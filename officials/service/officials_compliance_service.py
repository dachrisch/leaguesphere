...
def _best_valid_rank(history_entries, on_date):
    """history_entries: list[(created_at, rank)]. Returns the lowest
    (=best) rank among entries valid on `on_date`, or None if none are
    currently valid. A license is valid from its `created_at` date through
    (roughly) one year later - mirrors
    `OfficialLicenseHistory.valid_until()`/the correlated-subquery window
    used in `matchreport.service.model_wrapper`."""
    valid_ranks = [
        rank
        for created_at, rank in history_entries
        if created_at <= on_date and created_at > on_date - timedelta(days=365)
    ]
...
def compute_gameday_officials_compliance(
    ...
    if official_ids:
        histories = OfficialLicenseHistory.objects.filter(
            official_id__in=official_ids, license__name__in=LICENSE_LEVELS
        ).values("official_id", "license__name", "created_at")
        for h in histories:
            history_by_official[h["official_id"]].append(
                (h["created_at"], LICENSE_LEVELS.index(h["license__name"]))
            )
